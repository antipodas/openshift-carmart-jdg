...
public class PopulateCache implements SystemEventListener {
    ...
    public void startup() {
        ...
        try {
            ...
            carNumbers.add(c.getNumberPlate());
            cars.put(CarManager.encode(c.getNumberPlate()), c);
            log.info("Adding 50000 more cars: " +System.currentTimeMillis());
            for (int i = 0; i < 50000; i++) {
            	c = new Car("Seat 600", 1.6, CarType.HATCHBACK, "yellow", "matricula"+i, Country.GERMANY);
            	carNumbers.add(c.getNumberPlate());
            	cars.put(CarManager.encode(c.getNumberPlate()), c);
			} 
            ...
            log.info("Added 50000 more cars: " +System.currentTimeMillis());
        } catch (Exception e) {
